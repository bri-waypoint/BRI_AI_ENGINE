...
# Tab 1: Shannyn's Portfolio (existing properties)
# Tab 2: One-Off Property Search (for realtors/public)
...
def run_analysis_and_display(subject, radius, property_types,
                              use_rentcast, selected_type):
    """Run AI analysis and display results."""
    with st.spinner(
        "Searching BRI Vault + RentCast and analyzing "
        "with Claude AI... (30-60 seconds)"
    ):
        try:
            ...
            st.session_state['analyzed_address'] = (
                subject.get('address', '')
            )
    ...
    if total_leased >= 10:
        ...
        st.warning(f"⚠️ MEDIUM CONFIDENCE - {total_leased} recent leased comps")
    else:
...
with col3:
    st.metric("Shannyn's Properties",
              f"{len(subjects):,}")
with col4:
    st.metric("One-Off Searches",
              f"{stats.get('one_off_total', 0):,}")
with col5:
...
# ============================================================
# SEARCH PARAMETER SIDEBAR (shared between tabs)
# ============================================================
...
with st.sidebar:
    ...
    if use_rentcast:
        st.success(
            f"✅ {stats.get('rentcast_leased', 0):,} leased comps"
        )
...
with main_tab1:
    ...
    if not subjects:
        ...
        for s in subjects:
            ...
            sqft = (int(s.get('living_area', 0))
                    if s.get('living_area') else 0)
        ...
        if st.button("🤖 Analyze with AI",
            ...
            run_analysis_and_display(
                subject, radius, property_types,
                use_rentcast, selected_type
            )

        # Show results if already analyzed
        if (st.session_state.get('done') and
                'result' in st.session_state and
                st.session_state.get('analyzed_address') ==
                subject.get('address')):
            result = st.session_state['result']
            if result:
                st.markdown("---")
...
with main_tab2:
    ...
    st.markdown(
        "*Enter any property address for a one-time rental "
        "analysis. Perfect for realtors and individual requests.*"
    )
    ...
    with st.form("one_off_form", clear_on_submit=False):
        ...
        with col1:
            ...
            oo_city = st.text_input(
                "City *",
                value="Boise",
                help="Required"
            )
            oo_state = st.text_input(
                "State *",
                value="ID",
                help="Required"
            )
            oo_zipcode = st.text_input(
                ...
                help="Helps with geocoding accuracy"
            )
            oo_requester = st.text_input(
                "Requester Name",
                placeholder="e.g., John Smith - ABC Realty",
                help="Optional - for tracking purposes"
            )
        ...
        with col2:
            ...
            oo_year_built = st.number_input(
                ...
                value=2000,
                step=1,
                help="Optional - helps with analysis context"
            )
    ...
    if submitted:
        ...
        if not oo_address or not oo_city or not oo_state:
            ...
            with st.spinner(
                "Geocoding address and preparing analysis..."
            ):
                # Save to Supabase and geocode
            ...
            if not saved.get('latitude') or not saved.get('longitude'):
                ...
                st.success(
                    f"✅ Property geocoded successfully! "
                    f"Coordinates: {saved['latitude']:.4f}, "
    ...
    # ---- SHOW ANALYSIS BUTTON AFTER GEOCODING ----
    ...
    if recent:
        ...
        avail = {k: v for k, v in display_cols.items()
        ...
        st.markdown("**Re-analyze a recent property:**")
        recent_options = {
